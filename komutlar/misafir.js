...
exports.run = async (client, message, args) => {
 ...
 if (!kullanıcı) return message.channel.send(new Discord.MessageEmbed().addField((ayarlar.sunucuadı) , `Hey ! Lütfen Misafir Rolü Vericeğin Kullanıcığı Etiketle ${ayarlar.redemoji}`).setColor("2e0101").setFooter(message.author.tag ,message.author.avatarURL).setTimestamp());
  let user = message.mentions.users.first();
  let rol = message.mentions.roles.first()
  let member = message.guild.member(kullanıcı)
 
  member.roles.add((ayarlar.misafirrol)); //rol
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .addField(
      (ayarlar.sunucuadı),
      `${member.user} **Başarıyla Misafir Rolü Verildi** ${ayarlar.onayemoji}`
    )
    .setFooter(message.author.tag, message.author.avatarURL)
    .setTimestamp();
